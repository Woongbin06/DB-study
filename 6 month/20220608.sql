...
INSERT INTO MyUser VALUES(2, 'lee1502', '이황', '2022/05/01');
INSERT INTO MyUser VALUES(3, 'lee1536', '이이', '2022/05/02');
INSERT INTO MyUser VALUES(4, 'lee1597', '세종대왕', '2022/05/03');
...
INSERT INTO POST VALUES(2, 3, 4, '훈민정음', '훈민정음(訓民正音)은 백성을 가르치는 바른 소리라는 뜻이다. 훈민정음이라고 부르는 대상은 두 가지가 있는데, 그 하나는 1443년 음력 12월(양력 1444년 1월)에 세종대왕이 만든 한국어의 표기 체계, 즉 오늘날의 한글을 창제 당시에 부른 이름이고, 또 하나는 1446년 9월[1]에 발간된 책 이름이다. 여기서는 후자, 즉 훈민정음이라는 책을 설명한다.', '1583/01/01');
...
INSERT INTO Mycomment VALUES(1, 1, 2, '한산도 대첩(閑山島大捷) 혹은 견내량대첩(見乃梁大捷)은 1592년 8월 14일(선조 25년 음력 7월 8일) 통영 한산도 앞바다에서 조선 수군이 왜군을 크게 무찌른 해전', '2022/05/01', NULL);
INSERT INTO Mycomment VALUES(2, 1, 3, '이 전투에서 육전에서 사용하던 포위 섬멸 전술 형태인 학익진을 처음으로 해전에서 펼쳤다.', '2022/05/01', 1);
INSERT INTO Mycomment VALUES(3, 3, 2, '이이의 어머니 신사임당', '2022/05/01', NULL);
INSERT INTO Mycomment VALUES(4, 2, 1, '가나다라마바사', '2022/05/01', NULL);
--6번 문제
SELECT name
FROM Category;
--7번 문제
...
FROM post, myuser
WHERE post.user_seq = myuser.user_seq AND post.content LIKE '%신사임당%' OR post.user_seq LIKE '신사임당';
--9번 문제
